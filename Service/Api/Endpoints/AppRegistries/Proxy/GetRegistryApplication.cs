...
/// <summary>
/// Proxies GET /api/applications/{packageId} to the specified App Registry.
/// </summary>
public class GetRegistryApplication(IRegistryProxyService proxyService)
    : EndpointWithoutRequest
{
    public override void Configure()
    {
        Get("app-registries/{registryId}/applications/{packageId}");
        Summary(s =>
        {
            s.Summary = "Get a single application from a registry.";
            s.Description = "Proxies the request to the external App Registry using the stored API key.";
            s.Response(200, "Application returned as JSON.");
    ...
    public override async Task HandleAsync(CancellationToken ct)
    {
        ...
        var json = await proxyService.GetApplicationAsync(registryId, packageId, ct);
