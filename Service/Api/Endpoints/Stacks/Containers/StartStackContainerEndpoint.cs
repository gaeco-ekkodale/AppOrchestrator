...
/// <summary>
/// Starts one container of a stack.
/// </summary>
public class StartStackContainerEndpoint(
    IDockerContainerService dockerContainerService)
    : Endpoint<StackContainerRouteParams>
{
    public override void Configure()
    {
        Post("stacks/{projectName}/containers/{containerId}/start");
        Description(x => x.Accepts<StackContainerRouteParams>());
        Summary(s =>
        {
            s.Summary = "Start container.";
            s.Description = "Starts a single Docker container that belongs to the compose project identified by projectName.";
            s.Response(204, "Container started successfully.");
    ...
    public override async Task HandleAsync(StackContainerRouteParams req, CancellationToken ct)
    {
        ...
        await dockerContainerService.StartContainerAsync(projectName, containerId, ct);
