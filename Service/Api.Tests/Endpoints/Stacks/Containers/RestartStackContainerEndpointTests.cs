...
public class RestartStackContainerEndpointTests
{
    private readonly IDockerContainerService _dockerContainerService;
    private readonly RestartStackContainerEndpoint _endpoint;

    public RestartStackContainerEndpointTests()
    {
        _dockerContainerService = Substitute.For<IDockerContainerService>();
        _endpoint = FastEndpoints.Factory.Create<RestartStackContainerEndpoint>(_dockerContainerService);
    }

    [Fact]
    public async Task HandleAsync_ReturnsNoContent_WhenRestartSucceeds()
    {
        _dockerContainerService.RestartContainerAsync("stack-a", "api", Arg.Any<CancellationToken>())
    ...
    {
        _dockerContainerService.RestartContainerAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<CancellationToken>())
